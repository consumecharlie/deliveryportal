...
describe("POST /api/portal/[token]/message", () => {
  beforeEach(() => {
    ...
    vi.mocked(prisma.delivery.findFirst).mockResolvedValue({ senderEmail: "pm@consume-media.com" } as never);
    vi.mocked(loadPortal).mockResolvedValue({
      timeline: { projects: [{ listId: "list-1", name: "Spring Launch" }] },
    } as never);
  ...
  it("falls back to a DM when the project is not in this client's portal", async () => {
    const order = trackOrder();
    vi.mocked(loadPortal).mockResolvedValue({ timeline: { projects: [] } } as never);
