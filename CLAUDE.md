...
**Tables.** `PortalAccess` (token per client folder, `revokedAt`), `ProjectChannel` (internal Slack channel per project list, `autoMatched`, `confirmedBy`), `FeedbackConfirmation` (one row per "All feedback is in" press, `undoneAt` set on undo; newest row per delivery wins; `deliveryId` is nullable and `feedbackDeadlineTaskId` is the key for a task-only confirmation), `PortalView` (append-only open log per access and per delivery), `PortalReminder` (idempotency log keyed `deliveryId + kind + sentOn` Eastern date), `PortalMessage` (reach-out form notes, persisted before Slack). `Delivery.feedbackWindows` is snapshotted at send time for the fallback deadline.

**Page model (redesign).** `loadPortalPage(access, focusListId?)` in `portal-data.ts` reads every delivery for the client plus the newest confirmations, the client's ClickUp folder listing and the live per-list payload, then hands them to the pure `buildPortalPage` (`src/lib/portal-page.ts`), which returns `PortalPageModel` (`portal-page-model.ts`, the contract with the UI). Deliverables are keyed by `Delivery.parentTaskId` plus a variant stem (`deliverableKey` in `portal-labels.ts`): "Video Edit01" and "Snippets Edit01" under one parent are separate rows, "Edit V1" / "Edit V2" / "Final Deliverables" stack as versions; rows without a parent, or whose parent is only a phase word (`PHASE_ONLY_WORDS`: post-production, pre-production, production, design, editing, animation, post, pre, and spelling variants), fall back to `family:<type family>` and take the share task label as their title. **Projects come from the client's ClickUp folder, not from past deliveries**, so an active project appears from kickoff with its roadmap and fills in deliverables as we send them: `selectProjectLists` (`src/lib/portal-projects.ts`) keeps an active list when it has at least one Delivery Deadline milestone or at least one delivery, and an archived list only when it has deliveries (their roadmaps are history and listing every archived list would cost a ClickUp call each). The folder listing is cached in `DashboardCache` under `portal:folder:<clientFolderId>` by `getClientFolderLists` with the same 5 minute stale-while-revalidate treatment as the live payloads. A discovered list's ClickUp name wins over the stored `projectName`; archived projects keep the stored name because we never list them. Client names are stripped from project labels (`stripClientPrefix` in `portal-labels.ts`): ClickUp lists are named "<Client> <Project>" and the client already knows whose portal it is, so "Stack Overflow BVAS Talking Head Product Videos" shows as "BVAS Talking Head Product Videos". Only a full client-name prefix at a word boundary counts, so "Stack BVAS" keeps its name when the client is "Stack Overflow"; the remainder must be at least 3 characters and not all digits. A project with no deliveries yet shows an empty table reading "Nothing shared yet". `BuildPortalPageInput.discovered` is optional: omit it and the page is delivery-driven exactly as before. A project is `completed` only when its ClickUp list is archived; a list with every share task closed stays in progress ("next deliverable not scheduled yet") because lists keep gaining share tasks. Attention lists real deadlines before estimates; an archived project's deliverables are always review state `none` and never appear there. Feedback tasks pair to a delivery through `pairFeedbackTask` (`portal-live.ts`), under the delivery's parent: 1. same type; 2. same type minus version markers; 3. most identity tokens shared between the share task variant and the feedback task name (`deliverableIdentityTokens` / `nameTokens` in `portal-labels.ts`: "Video" vs "Snippets", generic words and version markers dropped); 4. the parent's only feedback task; 5. shared version markers between the types ("LoC Edit V2" vs "Edit V2"), else an open task; then the list-wide type map. Ties: open first, then the soonest due date on or after the send date. Several episodes share the type "Edit V1" and their feedback tasks are typed "LoC ...", so the type alone never suffices. `parentTaskId`/`parentTaskName`/`shareTaskName` are written at send time by `resolveShareIdentity` (`delivery-identity.ts`) and were backfilled with `scripts/backfill-delivery-parents.ts`. Labels come from `portal-labels.ts` (`deliverableTitle` strips department prefixes, `variantLabel` strips "Share ... with Client"; links are labelled by their anchor text in the sent message via `extractLinkTexts` + `cleanLinkText`, typed by `linkKind` with a host `linkHint`; review wording is `reviewLabel` by state and `reviewMode`, "approval" when the paired task name says Approval or the type says Final). A focused list narrows `projects` and `attention` but `counts` stay client-wide. The menu bar shows the uploaded logo as-is on the dark bar (a white or light wordmark, about 30px tall, no tile), or the client name when none is set. The logo is `ClientPreference.logoUrl`, surfaced as `clientLogoUrl`. Logos are uploaded from Settings > Client portal links to Vercel Blob (store `deliverable-portal-assets`, env `BLOB_READ_WRITE_TOKEN`) with the client-upload handshake at `POST /api/settings/client-logo` (`src/lib/client-logo.ts` rules: png/jpg/svg/webp, 2 MB, key `client-logos/<clientFolderId>.<ext>`, public, no random suffix, `?v=<ms>` cache-buster), then stored through `PATCH /api/settings/portal-access` (which also clears or accepts a pasted https URL); `clientDomain` stays in the model but the UI no longer uses it (no domain-based fallback in the header): it is the first usable recipient domain scanning deliveries newest first (primary, then cc; ours and personal mailboxes skipped; `deriveClientDomain`), falling back to the most common Project Contact domain across the client's in-progress lists, then completed ones (`contactDomains` in the live payload, `pickContactDomain`).

**Live deadlines and roadmap.** `src/lib/portal-live.ts` reads the Feedback Deadline tasks (Project Task Type 12) and the Delivery Deadline share tasks (type 11, closed included, parent names fetched once per distinct parent) per list through `getListTasksByDropdownField`, plus the list's due date and archived flag via `getList`, and caches the `LivePayload` (`{ version, feedback, feedbackByParent, milestones, wrapsUpMs, archived, contactDomains }`) in `DashboardCache` under `portal:fd:<listId>` for 5 minutes. Bump `LIVE_PAYLOAD_VERSION` when the shape changes; older rows are treated as misses. Stale-while-revalidate (a stale row is served and refreshed after the response via `after()`), misses fetched 4 at a time. Confirm and undo call `invalidateLiveFeedback(listId)`. A deliverable is awaiting only while its paired Feedback Deadline task is `waiting on client` (`awaitingClient`); a task in any other status, or no paired task at all, reads "Delivered", and a milestone is `in-review` under the same rule. The portal never infers an obligation ClickUp is not asserting. A deadline computed from the send date plus the feedback window (`src/lib/portal-deadline.ts`) is always an estimate and only appears when an awaiting task carries no due date.
...
**Action items without a delivery.** "Needs your review" is built from ClickUp, not only from what we sent. Any Feedback Deadline task whose status is `waiting on client` becomes an attention item (`awaitingClient` on `LiveFeedbackTask`), so a share task completed outside the portal (a details request, say) still puts the ball in the client's court. When a delivery exists the item keeps its links, versions and confirm button; when it does not, `PortalAttentionItem.deliveryId` is null, `feedbackTaskId` carries the ClickUp task, the row shows "We are waiting on your details" instead of a review link, and confirm and undo accept `{ feedbackTaskId }` (scoped by checking the task appears in the token's own projects, via `findPortalFeedbackTask`). Task-only rows never appear in the deliverables table, and an undated task produces no item. Titles come from `feedbackTaskTitle` (strips "Confirm" and the Feedback Received / Approval tails).

**Do not run `prisma db push` against this database without reading the diff.** Another branch (`portal-native-send`) owns a `Connection` table that this schema does not describe, so a blind push proposes to drop it. Apply portal schema changes with targeted SQL (or add the missing model first) and verify with `prisma migrate diff` that nothing destructive is left.
